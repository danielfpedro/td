<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Cards Type'), ['action' => 'edit', $cardsType->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Cards Type'), ['action' => 'delete', $cardsType->id], ['confirm' => __('Are you sure you want to delete # {0}?', $cardsType->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Cards Types'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Cards Type'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Cards'), ['controller' => 'Cards', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Card'), ['controller' => 'Cards', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="cardsTypes view large-9 medium-8 columns content">
    <h3><?= h($cardsType->name) ?></h3>
    <table class="vertical-table">
        <tr>
            <th><?= __('Name') ?></th>
            <td><?= h($cardsType->name) ?></td>
        </tr>
        <tr>
            <th><?= __('Id') ?></th>
            <td><?= $this->Number->format($cardsType->id) ?></td>
        </tr>
        <tr>
            <th><?= __('Created') ?></th>
            <td><?= h($cardsType->created) ?></td>
        </tr>
        <tr>
            <th><?= __('Modified') ?></th>
            <td><?= h($cardsType->modified) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Cards') ?></h4>
        <?php if (!empty($cardsType->cards)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($cardsType->cards as $cards): ?>
            <tr>
                <td><?= h($cards->id) ?></td>
                <td><?= h($cards->name) ?></td>
